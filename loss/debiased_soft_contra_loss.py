...
#         # w=pos_mask+neg_mask # new loss version 13
#         # log_w=torch.log(w) # new loss version 13
#         # log_w[log_w==float("-inf")]=0  # new loss version 13
#         # log_prob = log_w + logits - torch.log((w*exp_logits).sum(1, keepdim=True) + 1e-9) # new loss version 13
#         # old version 13
#         # log_prob = (torch.log((pos_mask+neg_mask)*exp_logits)/(((pos_mask+neg_mask)*exp_logits).sum(1, keepdim=True) + 1e-9))
#         # sum_mask = mask.sum(1)#  new version 13 
        
#         # compute mean of log-likelihood over positive
#         # version 14
#         log_prob = logits-torch.log(exp_logits.sum(1, keepdim=True) + 1e-9)
#         # sum_mask = (pos_mask+neg_mask).sum(1)  #  old version 13  + new version 14 
#         sum_mask = mask.sum(1)#  new version 12 
#         sum_mask[sum_mask == 0] = 1
...
#         else:
#             mean_log_prob_pos = ()
#             mean_log_prob_pos = (mask * log_prob).sum(1) / sum_mask  #  new version 13 new version 12
#             # mean_log_prob_pos = (pos_mask * log_prob).sum(1) / sum_mask # old version 13 new version 14
...
#     def __init__(self, temperature=0.07, contrast_mode='all',
#                  base_temperature=0.07):
#         super(DebiasSoftConLoss, self).__init__()
#         self.temperature = temperature
#         self.contrast_mode = contrast_mode
#         self.base_temperature = base_temperature
