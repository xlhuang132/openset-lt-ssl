...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_encoding=False,return_projected_feature=False,classifier=False, aux=False):
        if classifier:
            return self.fc(x)
        if return_projected_feature:# encoding l2 norm 
            x = self.projector(x,normalized=True)
            return x
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
